const entrepreneurs = [
  ...
  const nameArray = entrepreneurs.map(entrepreneur => `${entrepreneur.first} ${entrepreneur.last}`);
  ...
  thisYear = 2023
  for (let i = 0; i < entrepreneurs.length; i++) {
    console.log(entrepreneurs[i].first + " " + entrepreneurs[i].last + " " + "aurait aujourd'hui: " + (2023 - entrepreneurs[i].year) + " ans.");
  }
